...
function getLogoApiEndpoint() {
  return "https://logica-tiendaenlina.onrender.com/api/empresa/logo";
}
...
export default function ConfiguracionPersonalizada() {
  ...
  useEffect(() => {
    fetch("https://logica-tiendaenlina.onrender.com/api/admin/personalizar")
    ...
    fetch("https://logica-tiendaenlina.onrender.com/api/admin/config-entrega?id_admin=1")
      .then(response => {
        if (!response.ok) {
  ...
  function guardarConfigVisual(section, config) {
    ...
    (async () => {
      try {
        const tipoUsuario = JSON.parse(localStorage.getItem("admin") || "{}").tipo_usuario || "";
        const resActual = await fetch(`https://logica-tiendaenlina.onrender.com/api/admin/personalizar`);
        let foundId = null;
        if (resActual.ok) {
        ...
        let res;
        if (foundId) {
          res = await fetch(`https://logica-tiendaenlina.onrender.com/api/admin/personalizar/${foundId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              "X-Tipo-Usuario": tipoUsuario.toLowerCase()
          ...
          res = await fetch(`https://logica-tiendaenlina.onrender.com/api/admin/personalizar`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "X-Tipo-Usuario": tipoUsuario.toLowerCase()
